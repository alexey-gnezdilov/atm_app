...
create or replace function log_apartment_registration()
                          returns trigger as $$
                          begin
                             insert into logs_apartment (message_details) values ('Добавлены новые апартаменты по адресу: ' || 'Город ' || NEW.city || 'Улица  ' || NEW.street || 'дом ' || NEW.house_number);
                             return NEW;
                          end;
$$ language plpgsql;
...
create trigger new_apartment_registration_trigger
    after insert on address_info
    for each row execute function log_apartment_registration();